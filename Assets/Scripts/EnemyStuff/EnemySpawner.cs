...
namespace EnemyStuff
{
    public class EnemySpawner : MonoBehaviour
    {
            ...
            if (PlayerController.contPlayer.gameObject.activeSelf)
                if (_currentWave < waves.Count)
                {
                    ...
                    if (_spawnTimer <= 0)
                    {
                        ...
                        var newEnemy = EnemyPoolManager.PoolEnemys.wavePools[waves[_currentWave].enemyToSpawn]
                            .GetPooledGameObject();
        ...
        {
            _currentWave++;
            if (_currentWave >= waves.Count) _currentWave = waves.Count - 1;
