...
namespace Objects
{
    public class BasicObject : MonoBehaviour
    {
        ...
        {
            var p = DmgTypeSfxGroupController.Instance.sfxControllers[0].player.FeedbacksList[0];
            switch (dt)
            {
                case "Deathray":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[0].player.FeedbacksList[0];
                    p.Play(transform.position);
                    break;
                case "Acid":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[1].player.FeedbacksList[0];
                    p.Play(transform.position);                    break;
                case "Fire":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[2].player.FeedbacksList[0];
                    p.Play(transform.position);                    break;
                case "Energy":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[3].player.FeedbacksList[0];
                    p.Play(transform.position);                    break;
                case "Mind":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[4].player.FeedbacksList[0];
                    p.Play(transform.position);                    break;
                case "Solid":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[5].player.FeedbacksList[0];
                    p.Play(transform.position);                    break;
                case "Slice":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[6].player.FeedbacksList[0];
                    p.Play(transform.position);                    break;
                case "Stab":
                    p = DmgTypeSfxGroupController.Instance.sfxControllers[7].player.FeedbacksList[0];
                    p.Play(transform.position);                    break;
            }
        }
